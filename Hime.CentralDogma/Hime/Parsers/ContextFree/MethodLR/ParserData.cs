namespace Hime.Parsers.CF.LR
{
    ...
    public class RNGLR1ParserData : LRParserData
    {
        ...
        public RNGLR1ParserData(ParserGenerator generator, CFGrammar gram, Graph graph) : base(generator, gram, graph) { }
        ...
        protected void Export_State_ReducsOnTerminal(ItemSet State)
        {
            ...
            {
                if (!first) p_Stream.Write(", ");
                p_Stream.Write("new ushort[3] { 0x" + Reduction.OnSymbol.SID.ToString("x") + ", 0x" + p_Grammar.Rules.IndexOf(Reduction.ToReduceRule).ToString("X") + ", 0x" + Reduction.ReduceLength.ToString("X") + " }");
                first = false;
            }
            p_Stream.WriteLine(" };");
        }
