...
pub fn write(
    ...
    if !suppress_module_doc {
        writeln!(writer, "//! Module for the lexer and parser for `{}`", &name)?;
    ...
    if compress_automata {
        ...
        writeln!(writer, "/// Static resource for the serialized lexer automaton")?;
        writeln!(writer, "static LEXER_AUTOMATON: &[u8] = include_bytes!(\"{bin_name}\");")?;
    }
    writeln!(writer)?;
    ...
    for (index, context) in grammar.contexts.iter().enumerate().skip(1) {
        ...
        writeln!(writer, "pub const CONTEXT_{}: u16 = 0x{:04X};", to_upper_case(context), index)?;
    }
    writeln!(writer)?;
    ...
    for terminal_ref in expected.content.iter().skip(2) {
        ...
        writeln!(writer, "        name: \"{}\"", terminal.value.replace('"', "\\\""))?;
        write!(writer, "    }}")?;
    }
    writeln!(writer)?;
    writeln!(writer, "];")?;
