...
impl SPPFNode {
    ...
    /// Gets this node as a replaceable node
    pub fn as_replaceable(&self) -> &SPPFNodeReplaceable {
        match self {
            &SPPFNode::Normal(ref _node) => panic!("Expected replaceable node"),
            &SPPFNode::Replaceable(ref node) => node
        }
    }
