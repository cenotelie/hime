...
namespace Hime.CentralDogma.Grammars.ContextFree.LR
{
    ...
    abstract class ParserGenerator : Hime.CentralDogma.Grammars.ParserGenerator
    {
        ...
        protected ParserGenerator(string name, Reporting.Reporter reporter)
		{
			this.name = name;
        ...
        public ParserData Build(Grammar grammar)
		{
            this.ReportInfo("Constructing " + this.name + " data ...");
			this.graph = this.BuildGraph(grammar as CFGrammar);
            Close();
            this.ReportInfo(this.graph.States.Count.ToString() + " states explored.");
            this.ReportInfo("Done !");
			return BuildParserData(grammar as CFGrammar);
		}
        ...
        private void Report(Reporting.Entry entry)
        {
            lock (reporter)
            {
