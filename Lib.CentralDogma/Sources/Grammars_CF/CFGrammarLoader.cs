...
namespace Hime.CentralDogma.Grammars.ContextFree
{
    class CFGrammarLoader : GrammarLoader
    {
        ...
        private Automata.NFA Compile_Recognize_terminal_def_atom_ublock(ASTNode node)
        {
            ...
            final.StateEntry = final.AddNewState();
            final.StateExit = final.AddNewState();
            Hime.Redist.Symbol token = node.Symbol;
        ...
        private Automata.NFA Compile_Recognize_terminal_def_atom_span(ASTNode node)
        {
            ...
            int spanEnd = 0;
            // Get span begin
            Hime.Redist.Symbol child = node.Children[0].Symbol;
            spanBegin = System.Convert.ToInt32(child.Value.Substring(2), 16);
            // Get span end
            child = node.Children[1].Symbol;
        ...
        private Automata.NFA Compile_Recognize_terminal_def_atom_name(ASTNode node)
        {
            ...
            if (Ref == null)
            {
                reporter.Error(resName + " @" + node.Position + " Cannot find terminal " + token.Value);
                Automata.NFA Final = new Automata.NFA();
                Final.StateEntry = Final.AddNewState();
                Final.StateExit = Final.AddNewState();
        ...
        private Automata.NFA Compile_Recognize_terminal_definition(ASTNode node)
        {
            ...
            if (symbol.Name == "range")
            {
                ...
                uint min = System.Convert.ToUInt32(node.Children[1].Symbol.Value);
                uint max = min;
                if (node.Children.Count > 2)
                    max = System.Convert.ToUInt32(node.Children[2].Symbol.Value);
        ...
        {
            Hime.Redist.Symbol symbol = node.Children[0].Symbol;
            TextTerminal terminal = grammar.GetTerminalByName(symbol.Value) as TextTerminal;
            if (terminal == null)
            {
                Automata.NFA nfa = Compile_Recognize_terminal_definition(node.Children[1]);
                terminal = grammar.AddTerminalNamed(symbol.Value, nfa);
                nfa.StateExit.Item = terminal;
            }
            else
            {
                reporter.Error(resName + " @" + node.Children[0].Position + " Overriding the definition of terminal " + symbol.Value);
            }
            return terminal;
        }
        ...
        {
            Hime.Redist.Symbol token = node.Children[0].Symbol;
            CFRuleBodySet defs = new CFRuleBodySet();
            if (token.Value == "emptypart")
            {
            ...
            else
            {
                ...
                {
                    reporter.Error(resName + " @" + node.Children[0].Position + " Unrecognized symbol " + token.Value + " in rule definition");
                    defs.Add(new CFRuleBody());
                }
            }
            return defs;
        ...
        private CFRuleBodySet Compile_Recognize_rule_sym_ref_template(CompilerContext context, ASTNode node)
        {
            ...
            {
                reporter.Error(resName + " @" + node.Children[0].Position + " Meta-rule " + name + " does not exist with " + paramCount.ToString() + " parameters");
                defs.Add(new CFRuleBody());
                return defs;
            }
