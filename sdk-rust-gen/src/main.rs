...
pub fn get_latest_categories() -> Vec<UnicodeCategory> {
    ...
    categories
        ...
        .unwrap()
        .spans
        .push(UnicodeSpan::new(current.1, last_cp));
...
/// Generates the Unicode blocks tests
...
pub fn generate_categories_db(categories: &Vec<UnicodeCategory>) -> Result<(), Error> {
    let mut aggregated = HashMap::new();
    for category in categories.iter() {
        let first: u8 = (&category.name as &AsRef<[u8]>).as_ref()[0];
        if !aggregated.contains_key(&first) {
            aggregated.insert(first, Vec::<&UnicodeCategory>::new());
        }
        aggregated.get_mut(&first).unwrap().push(category);
    }

    let mut file = File::create("categories.rs").unwrap();
    write!(&mut file, "/*\n")?;
    write!(&mut file, " * WARNING: this file has been generated by\n")?;
    write!(&mut file, " * Hime Parser Generator\n")?;
    write!(&mut file, " */\n")?;
    write!(&mut file, "\n")?;
    write!(&mut file, "//! Contains the supported Unicode categories\n")?;
    write!(&mut file, "\n")?;
    write!(&mut file, "use std::collections::HashMap;\n")?;
    write!(&mut file, "use super::UnicodeCategory;\n")?;
    write!(&mut file, "use super::UnicodeSpan;\n")?;
    write!(&mut file, "\n")?;
    for category in categories.iter() {
        let name = helper::to_upper_case(&category.name);
        write!(&mut file, "/// The unicode category {0}\n", &category.name)?;
        write!(
            &mut file,
            "pub const {:}: &'static str = \"{:}\";\n",
            name, category.name
    ...
    for (first, _content) in aggregated.iter() {
        let name = ((*first) as char).to_ascii_uppercase();
        write!(&mut file, "/// The unicode category {0}\n", name)?;
        write!(&mut file, "pub const {0}: &'static str = \"{0}\";\n", name)?;
        write!(&mut file, "\n")?;
    }
    write!(
        &mut file,
    ...
    for category in categories.iter() {
        let name = helper::to_snake_case(&category.name);
        let mutable = if category.spans.is_empty() {
            ""
        } else {
            " mut"
        };
        write!(
            &mut file,
            "        let{:} {:} = UnicodeCategory::new(\"{:}\".to_owned());\n",
            mutable, name, category.name
        )?;
        for span in category.spans.iter() {
            write!(
                &mut file,
                "        {:}.add_span(UnicodeSpan::new({:#X}, {:#X}));\n",
                name, span.begin.value, span.end.value
            )?;
        }
    }
    for (first, content) in aggregated.iter() {
        let name = (*first) as char;
        let var = name.to_ascii_lowercase();
        write!(
            &mut file,
            "        let mut {:} = UnicodeCategory::new(\"{:}\".to_owned());\n",
            var, name
        )?;
        for category in content.iter() {
            write!(
                &mut file,
                "        {:}.aggregate(&{:});\n",
                var,
                helper::to_snake_case(&category.name)
            )?;
        }
    }
    write!(&mut file, "        let mut map = HashMap::new();\n")?;
    for category in categories.iter() {
        let key = helper::to_upper_case(&category.name);
        let var = helper::to_snake_case(&category.name);
    ...
    write!(&mut file, "}}\n")?;
    write!(&mut file, "\n")?;
    write!(&mut file, "/// Retrieves the definition of a category\n")?;
    write!(
        &mut file,
